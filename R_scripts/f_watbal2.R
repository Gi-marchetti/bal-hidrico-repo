...
watball <- function(t, p, lat, CC){
  ...
  while(it < 20) { #should work with less than 20
    ...
    ppa2 <- ppa
    for(h in n){
      if(h == n){ #if last Dec, takes p_pet from Jan and Feb
        if(p_pet[1] >= 0) {
  ...
  if(length(w) > 0){
    if(w[1] == 1) {rec <- 1:n} else {rec <- c(w[1]:n, 1:(w[1]-1))} #order of months
    rnf <- rep(0,n)
    seguir <- TRUE
    # initial
    for(i in rec){
  ...
  rownames(balhid)[n+1] <- 'TOTAL'
  
  colnames(balhid) <-  c('T', 'P', 'PET', 'TEAW', 'PALW', 'ST',
                         'i_ST', 'RET', 'DEF', 'EXD', 'r', 'rP')
  
  balhid

}
wb <- watball(t = tabela[,1], p = tabela[,2], lat = -45, CC = 100)
